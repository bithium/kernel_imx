 ...
 * 
 * Bithium modified ILI2117A touch screen controller driver
 * 
 * Based on Adam Ford <aford173@gmail.com> patch (https://patchwork.kernel.org/patch/10849877/)
 * 
 * */
...
#define DEFAULT_MAX_X		(1 << 12)
#define DEFAULT_MAX_Y		(1 << 12)

/* Touchscreen commands */
#define TOUCH_INFO		0x00

struct finger {
	u8 xy;      // upper 4bits of x and y
	u8 x;       // lower 8bit of x
	u8 y;       // lower 8bit of y
	u8 c_sum;
} __packed;
...
struct ili2117a {
	...
	struct touchdata tsdata;
};

static int ili2117a_read_reg(struct i2c_client *client, u8 reg, void *buf, size_t len)
{
...
static irqreturn_t ili2117a_irq(int irq, void *irq_data){
	...
	struct touchdata *tsdata = &priv->tsdata;
	...
	error = ili2117a_read_reg(priv->client, TOUCH_INFO, (uint8_t *)tsdata, sizeof(struct touchdata));   
	...
	// only packets with id 0x5A are valid
	if(tsdata->packet_id != 0x5A){
		return IRQ_HANDLED;
	}	

	for (i = 0; i < priv->max_touch; i++) {
		active = false;
		finger = &tsdata->finger[i];

		x = finger->x | (((finger->xy & 0xf0) >> 4) << 8);
		y = finger->y | ((finger->xy & 0x0f) << 8);	

		/* on release x and y have 0xFFF */
		if (x != 0xFFF && y != 0xFFF) {
			active = true;	
		}        
				       ...
				       const struct i2c_device_id *id)
{
	...
	if (!priv || !input) {
		error = -ENOMEM;
        	dev_err(dev, "Unable to allocate memory, err: %d\n", error);
		goto err_free_mem;
	}
	...
	error = device_property_read_u32(dev, "ili2117a,poll-period", &poll_period);
	priv->poll_period = error ? DEFAULT_POLL_PERIOD : poll_period;

	error = device_property_read_u32(dev, "ili2117a,max-touch", &max_touch);
	if (max_touch > MAX_TOUCHES)
		max_touch = MAX_TOUCHES;
	priv->max_touch = error ? MAX_TOUCHES : max_touch;

	dev_info(dev, "Maximum points %d \n",priv->max_touch);

	error = device_property_read_u32(dev, "ili2117a,max-x", &max_x);
	if (max_x > DEFAULT_MAX_X) 
		max_x = DEFAULT_MAX_X;
    
	priv->prop.max_x = error ? DEFAULT_MAX_X : max_x;

	error = device_property_read_u32(dev, "ili2117a,max-y", &max_y);
	if (max_x > DEFAULT_MAX_Y) 
		max_x = DEFAULT_MAX_Y;
    
	priv->prop.max_y = error ? DEFAULT_MAX_Y : max_y;
        
	dev_info(dev, "Touch size x=%d, y=%d \n", priv->prop.max_x, priv->prop.max_y);	
	...
	/* Single touch */
	input_set_abs_params(input, ABS_X, 0, priv->prop.max_x + 1, 0, 0);
	input_set_abs_params(input, ABS_Y, 0, priv->prop.max_y + 1, 0, 0);

	/* Multi touch */
	input_mt_init_slots(input, priv->max_touch, 0);
	input_set_abs_params(input, ABS_MT_POSITION_X, 0, priv->prop.max_x + 1, 0, 0);
	input_set_abs_params(input, ABS_MT_POSITION_Y, 0, priv->prop.max_y + 1, 0, 0);
